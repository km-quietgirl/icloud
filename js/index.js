...
//事情处理
		...
		return {
			...
			link:function($scope,el){
				...
				$(el).on("click","input",function(){			//右边单击获取焦点
	...
	app.directive("myUl",function(){
		return {
			...
			link:function($scope,el){
				...
				$(el).on("click","input",function(e){	  //双击获取焦点
					$(el).find("input").attr("readonly","readonly")
					$(this).removeAttr("readonly")
					$(this).focus();	
				})
...
//						$(".tip").eq(0).addClass("listactive");	
	...
	app.controller("mainCtrl",["$scope",function($scope){
		...
		if(localStorage.reminder&&localStorage.reminder.length!=0){
			...
			color:"purple",
			todos:[
			{"name":"水浒传","state":1,"date":"10"},
			{"name":"西游记","state":1,"date":"10"},
			{"name":"hh1","state":0,"date":"10"}
			]
		}
			]
			$scope.cu=0
